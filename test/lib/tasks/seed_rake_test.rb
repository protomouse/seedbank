...
describe 'Seedbank rake.task' do
  ...
  describe "common seeds in the root directory" do
    ...
    Dir[File.expand_path('../../../dummy/db/seeds/*.rb', __FILE__)].each do |seed_file|
      seed = File.basename(seed_file, '.rb')
  ...
  describe "db:seed:common" do
    ...
    it "is dependent on the common seeds and db:seed:original" do
      prerequisite_seeds = Dir[File.expand_path('../../../dummy/db/seeds/*.rb', __FILE__)].sort.map do |seed_file|
        ['db', 'seed', File.basename(seed_file, '.rb')].join(':')
  ...
  describe "environment seeds" do
    ...
    Dir[File.expand_path('../../../dummy/db/seeds', __FILE__) + '/*/'].each do |environment_directory|
      ...
      describe "seeds in the #{environment} environment" do
        ...
        Dir[File.expand_path("../../../dummy/db/seeds/#{environment}/*.rb", __FILE__)].each do |seed_file|
          seed = File.basename(seed_file, '.rb')
      ...
      describe "db:seed:#{environment}" do
        ...
        it "is dependent on the seeds in the environment directory" do
          prerequisite_seeds = Dir[File.expand_path("../../../dummy/db/seeds/#{environment}/*.rb", __FILE__)].sort.map do |seed_file|
            ['db', 'seed', environment, File.basename(seed_file, '.rb')].join(':')
