namespace :db do
  ...
  namespace :seed do
    ...
    common_dependencies = glob_seed_files_matching('*.rb').sort.map { |seed_file| seed_task_from_file(seed_file) }

    desc "Load the seed data from db/seeds.rb and db/seeds/*.rb."
    ...
    glob_seed_files_matching('/*/').each do |directory|
      ...
      environment_dependencies = glob_seed_files_matching(environment, '*.rb').sort.map { |seed_file| seed_task_from_file(seed_file) }

      desc "Load the seed data from db/seeds.rb, db/seeds/*.rb and db/seeds/#{environment}/*.rb."
  ...
  desc 'Load the seed data from db/seeds.rb, db/seeds/*.rb and db/seeds/ENVIRONMENT/*.rb. ENVIRONMENT is the current environment in Rails.env.'
